...
def main():
    ...
    for variant in variations:
        for setting in variant["settings"]:
            ...
            # Construct paths
            output_path = f"results/{args.folder}/{'/'.join(components)}/exp_self"
            module_path = f"ray.experimental.tp.src.main.{'.'.join(components)}"
    ...
    if not os.getcwd().endswith("python/ray/experimental/tp"):
        logger.error("Not running from python/ray/experimental/tp directory")
        sys.exit(1)
